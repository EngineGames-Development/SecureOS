...
void init_memory() {
  heap_start = (unsigned int)&end;
  if (heap_start & 0xF) {
    heap_start = (heap_start & 0xFFFFFFF0) + 0x10;
  }
  ...
  while (1) {
    ...
    if (current_block->is_free && current_block->size >= size) {
      ...
      if (current_block->size > needed_space + 32) {
        unsigned int next_address = current_address + needed_space;
...
void kfree(void *ptr) {
  ...
  unsigned int block_address = (unsigned int)ptr - sizeof(struct memory_block);
